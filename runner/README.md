...
# train_step on CPU
python src/train_step.py trainer=cpu

# train_step on GPU
python src/train_step.py trainer=gpu
```

Train model with chosen experiment configuration from [configs/experiment/](configs/experiment/)

```bash
python src/train_step.py experiment=experiment_name
```

You can override any parameter from command line like this

```bash
python src/train_step.py trainer.max_epochs=20 datamodule.batch_size=64
```
...
```bash
python train_step.py trainer.max_epochs=20 model.optimizer.lr=1e-4
```

> **Note**: You can also add new parameters with `+` sign.

```bash
python train_step.py +model.new_param="owo"
```
...
```bash
# train_step on CPU
python train_step.py trainer=cpu

# train_step on 1 GPU
python train_step.py trainer=gpu

# train_step on TPU
python train_step.py +trainer.tpu_cores=8

# train_step with DDP (Distributed Data Parallel) (4 GPUs)
python train_step.py trainer=ddp trainer.devices=4

# train_step with DDP (Distributed Data Parallel) (8 GPUs, 2 nodes)
python train_step.py trainer=ddp trainer.devices=4 trainer.num_nodes=2

# simulate DDP on CPU processes
python train_step.py trainer=ddp_sim trainer.devices=2

# accelerate training on mac
python train_step.py trainer=mps
```
...
```bash
# train_step with pytorch native automatic mixed precision (AMP)
python train_step.py trainer=gpu +trainer.precision=16
```
...
```bash
# train_step model with Weights&Biases (link to wandb dashboard should appear in the terminal)
python train_step.py logger=wandb
```
...
```bash
python train_step.py experiment=example
```
...
```bash
python train_step.py callbacks=default
```
...
python train_step.py +trainer.gradient_clip_val=0.5

# run validation loop 4 times during a training epoch
python train_step.py +trainer.val_check_interval=0.25

# accumulate gradients
python train_step.py +trainer.accumulate_grad_batches=10

# terminate training after 12 hours
python train_step.py +trainer.max_time="00:12:00:00"
```
...
python train_step.py debug=default

# run 1 train_step, val and test loop, using only 1 batch
python train_step.py debug=fdr

# print execution time profiling
python train_step.py debug=profiler

# try overfitting to 1 batch
python train_step.py debug=overfit

# raise exception if there are any numerical anomalies in tensors, like NaN or +/-inf
python train_step.py +trainer.detect_anomaly=true

# log second gradient norm of the model
python train_step.py +trainer.track_grad_norm=2

# use only 20% of the data
python train_step.py +trainer.limit_train_batches=0.2 \
...
```yaml
python train_step.py ckpt_path="/path/to/ckpt/name.ckpt"
```
...
python train_step.py -m datamodule.batch_size=32,64,128 model.lr=0.001,0.0005
```
...
python train_step.py -m hparams_search=mnist_optuna experiment=example
```
...
```bash
python train_step.py -m 'experiment=glob(*)'
```
...
```bash
python train_step.py -m seed=1,2,3,4,5 trainer.deterministic=True logger=csv tags=["benchmark"]
```
...
```bash
>>> python train_step.py -m +x=1,2,3 tags=[]
